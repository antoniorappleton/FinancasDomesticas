...
/**
 * Scheduled Function: Daily at 9:00 AM Lisbon time
 */
      ...
      const promises = subs.map(async (sub) => {
        ...
        try {
          ...
          if (err.statusCode === 410 || err.statusCode === 404) {
            // Changed/Expired -> Delete from DB
            logger.info(
  ...
  cors(req, res, async () => {
    try {
      ...
      const payload = JSON.stringify({
        ...
        body: "Se estás a ver isto, o sistema funciona!",
        icon: "/icon-192.png",
      });

      await webpush.sendNotification(pushConfig, payload);
      res.send(`Sent test to ${subs.length} devices.`);
    } catch (e) {
      logger.error(e);
