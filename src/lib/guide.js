...
// === SVG ICONS ===
const ICONS = {
  helpGeneric: () => `<svg fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="guide-icon"><path stroke-linecap="round" stroke-linejoin="round" d="M9.879 7.519c1.171-1.025 3.071-1.025 4.242 0 1.172 1.025 1.172 2.687 0 3.712-.203.179-.43.326-.67.442-.745.361-1.45.999-1.45 1.827v.75M12 17.25h.007v.008H12v-.008z" /></svg>`,
...
const CONTENT = {
  general: {
    ...
    title: "Guia Geral",
    what: ["Registar movimentos diários e fixos", "Analisar as tuas finanças (Dashboard)", "Organizar categorias e orçamentos"],
    how: ["Usa o botão + para novos registos", "Navega no menu inferior para ver histórico e definições", "Toca nos cartões do Dashboard para ver detalhes"],
  ...
  dashboard: {
    ...
    how: ["Toca nos cartões grandes para ver gráficos detalhados", "Desliza para ver os widgets inferiores"],
    tips: ["Podes ocultar os mini-cards nas Definições se preferires um visual mais limpo."]
  },
  new: {
    icon: ICONS.plus(),
    ...
    tips: ["Usa descrições consistentes para que a app sugira categorias automaticamente no futuro."]
  },
  transactions: {
    icon: ICONS.list(),
    title: "Movimentos",
    what: ["Ver histórico completo", "Filtrar por texto, data ou meio", "Editar ou apagar registos"],
    how: ["Usa a barra de pesquisa ou os filtros no topo", "Carrega numa transação para editar os detalhes"],
    tips: ["Se a categoria estiver errada, corrige-a aqui para melhorar a inteligência da app."]
  },
  categories: {
    icon: ICONS.tags(),
    title: "Categorias",
    what: ["Criar e organizar categorias Pai/Filho", "Definir se é despesa, receita ou poupança"],
    how: ["Cria primeiro a categoria Pai, depois adiciona sub-categorias (Filhos)"],
    tips: ["Evita criar muitos 'Outros'. Nomes específicos ajudam nos relatórios."]
  },
  goals: {
    icon: ICONS.target(),
    title: "Objetivos",
    what: ["Definir metas de poupança", "Acompanhar o progresso visualmente"],
    how: ["Cria um objetivo, define o valor alvo e associa a uma categoria de poupança"],
    tips: ["Começa com objetivos pequenos e realistas para manter a motivação."]
  },
  settings: {
    icon: ICONS.gear(),
    title: "Definições",
    ...
    tips: ["A importação de PDF funciona melhor com extratos mensais de texto (não digitalizados)."]
  }
};
...
function getRouteKey(route) {
  ...
  if (route.includes('movimentos') || route.includes('transactions')) return 'transactions';
...
class Guide {
  ...
  static mountScreenButton() {
    ...
    let target = document.querySelector(".screen-header") || 
