...
export async function init({ outlet } = {}) {
  const sb = window.sb;
  const $ = (id) => (outlet && outlet.querySelector(`#${id}`)) || document.getElementById(id);
  ...
  if (accRes.error||regRes.error||pmRes.error||stRes.error||ttRes.error) {
    console.error(accRes.error||regRes.error||pmRes.error||stRes.error||ttRes.error);
    toast("Erro a carregar listas", false);
    return;
  }
  const TYPE_ID = Object.fromEntries((ttRes.data||[]).map(t => [t.code, t.id]));
  ...
  async function loadAccounts() {
    ...
    if (error || !data?.length) {
      ...
      $("tx-save") && (($("tx-save").disabled = true), ($("tx-save").title = "Cria pelo menos uma conta nas Definições."));
      return;
    }
    fill($("tx-account"), data, "name");
    fill($("tx-account-from"), data, "name");
    fill($("tx-account-to"), data, "name");
    if ($("tx-save")) { $("tx-save").disabled = false; $("tx-save").title = ""; }
  }
  await loadAccounts();
  ...
  async function loadPlainCategories(kind) {
    ...
    $("tx-category").innerHTML = rows.map(r => `<option value="${r.id}">${r.label}</option>`).join("");
  }
  ...
  async function bindExpenseDropdowns() {
    ...
    const { data: parents, error: e1 } = await sb
      ...
      .eq("kind","expense")
      .order("name");
    if (e1) { console.error(e1); toast("Erro a carregar categorias", false); return; }

    const pSorted = (parents||[]).sort((a,b)=>coll.compare(a.name,b.name));
    ...
    $("cat-parent").onchange = async () => {
      ...
      if (!pid) { $("cat-child").innerHTML = `<option value="">Subcategoria</option>`; return; }
      const { data: subs, error: e2 } = await sb
        ...
        .eq("kind","expense")
        .order("name");
      if (e2) { console.error(e2); toast("Erro a carregar subcategorias", false); return; }
      const sSorted = (subs||[]).sort((a,b)=>coll.compare(a.name,b.name));
      $("cat-child").innerHTML =
        `<option value="">Subcategoria</option>` +
        sSorted.map(s => `<option value="${s.id}">${s.name}</option>`).join("");
    };
    $("cat-child").onchange = () => {
  ...
  const currentType = () => (document.querySelector('input[name="tx-type"]:checked')?.value || "INCOME").toUpperCase();
  ...
  $("tx-save")?.addEventListener("click", async () => {
    try {
      ...
      if (t === "TRANSFER") {
        ...
        if (!from_account || from_account==="__none__" || !to_account || to_account==="__none__") throw new Error("Seleciona as duas contas.");
        if (from_account === to_account) throw new Error("As contas têm de ser diferentes.");
        ...
        ["tx-amount","tx-desc","tx-notes"].forEach(id => $(id) && ($(id).value=""));
        return;
      }

      // income / expense / savings
      const account_id = $("tx-account")?.value;
      if (!account_id || account_id==="__none__") throw new Error("Escolhe a conta.");
      ...
      const payload = {
        user_id: user.id,
        type_id, regularity_id, account_id, category_id,
      ...
      ["tx-amount","tx-desc","tx-loc","tx-notes"].forEach(id => $(id) && ($(id).value=""));
    } catch (e) {
      console.error(e);
