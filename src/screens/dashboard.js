...
export async function init({ sb, outlet } = {}) {
  ...
  function lastNMonthsKeys(n) {
    ...
    for (let i=n-1;i>=0;i--) out.push(yyyMmLocal(new Date(base.getFullYear(), base.getMonth()-i, 1)));
    return out;
  }
  ...
  async function getMonthlyViaView() {
    ...
    const map = new Map(
      data.map(r => [ monthKeyFromRow(r.month), {
  ...
  async function getMonthlyFallback() {
    ...
    return monthsKeys.map(k => ({ key:k, label:labelMonthPT(k), ...(agg.get(k)||{income:0,expense:0,savings:0,net:0}) }));
  }
  ...
  const netCum = [];
  const savCum = [];
  monthly.reduce((acc, m, i) => { netCum[i]=acc+m.net; return netCum[i]; }, 0);
  monthly.reduce((acc, m, i) => { savCum[i]=acc+Math.abs(m.savings); return savCum[i]; }, 0);
  ...
  mountChart("chart-cashflow", {
    ...
    data: {
      labels: monthly.map(m => m.label),
      datasets: [
        { label:"Saldo líquido (acum.)", data: netCum, tension:.25, borderWidth:2, fill:true },
        { label:"Poupanças (acum.)",     data: savCum, tension:.25, borderWidth:2, fill:false }
      ]
    },
    options: {
      responsive:true, maintainAspectRatio:false,
  ...
  try {
    ...
    (cats||[]).forEach(c => cParents.set(c.id, { name:c.name, parent_id:c.parent_id }));
  } catch {}
  ...
  const catPath = (id, rowCat=null) => {
    ...
    const c = cParents.get(id); if (!c) return "(Sem categoria)";
  ...
  mountChart("chart-fixed-donut", {
    type: "doughnut",
    data: { labels:["Fixas","Variáveis"], datasets:[{ data:[thisMonthAgg.fixed, thisMonthAgg.variable] }] },
  ...
  (() => {
    ...
    mountChart("chart-fixed-stacked", {
      ...
      data:{ labels: rows.map(r => r.label),
        datasets:[ { label:"Fixas", data: rows.map(r => r.fixed), stack:"exp" },
  ...
  try {
    ...
    const { data } = await sb.from("transactions").select("date,amount,type_id").gte("date", from).lt("date", to).order("date", {ascending:true});
    expMonth = (data||[]).filter(r => CODE_BY_ID.get(r.type_id) === "EXPENSE");
  } catch {}
  ...
  const cum = []; dayLabels.reduce((acc,k,i)=>(cum[i]=acc+(dayMap.get(k)||0),cum[i]),0);
  ...
  mountChart("chart-methods", {
    type:"bar",
    data:{ labels: Array.from(pmMap.keys()), datasets:[{ label:"Total", data: Array.from(pmMap.keys()).map(k=>pmMap.get(k)||0) }] },
    options:{ indexAxis:"y", responsive:true, maintainAspectRatio:false, scales:{ x:{ beginAtZero:true } } }
  });
  ...
  (async () => {
    ...
    const agg = new Map();
    rows.forEach(r => { const k=keyOf(r); agg.set(k, (agg.get(k)||0) + Number(r.amount||0)); });
    ...
    mountChart("chart-regularities", {
      ...
      options:{
        ...
        scales:{ y:{ beginAtZero:true } },
        interaction:{ mode:"index", intersect:false },
        plugins:{ tooltip:{ callbacks:{ label:(ctx)=>{ const v=Number(ctx.parsed.y||0); const p=((v/total)*100).toFixed(1)+"%"; return ` ${ctx.dataset.label}: € ${v.toLocaleString("pt-PT",{minimumFractionDigits:2})} (${p})`; } } } }
      }
    });
    ...
    if (legendEl) {
      legendEl.innerHTML = labels.map((lab,i)=> {
        ...
        return `<div class="rpt-legend__item"><span style="flex:1">${lab}</span><strong>€ ${v.toLocaleString("pt-PT",{minimumFractionDigits:2})}</strong><span style="color:#64748b">&nbsp;(${pct}%)</span></div>`;
      }).join("");
    }
  })();
  ...
  async function buildCategoryAnalysis() {
    let rows = [];
    try {
      const { data } = await sb.from("v_expense_by_category").select("category,total_expense").order("total_expense",{ascending:false});
      if (data?.length) rows = data.map(r => ({ category:r.category, total:Number(r.total_expense||0) }));
    } catch {}
    ...
    if (!rows.length) {
      ...
      const { data } = await sb.from("transactions").select("amount, categories(name,parent_id)").eq("type_id", expId).gte("date", fromISO);
      const parents = new Map(); try { const { data: cats } = await sb.from("categories").select("id,name"); (cats||[]).forEach(c => parents.set(c.id,c.name)); } catch {}
      const acc = new Map();
      (data||[]).forEach(r => {
      ...
      rows = Array.from(acc.entries()).map(([category,total])=>({category,total})).sort((a,b)=>b.total-a.total);
    }
    rows = rows.filter(r=>r.total>0);

    renderCategoryPie(rows, rows.reduce((s,r)=>s+r.total,0));
    renderCategoryList(rows, rows.reduce((s,r)=>s+r.total,0), 12);
  }

  function renderCategoryPie(rows, total) {
    const el = byId("chart-cat-pie"); const ctx = el?.getContext("2d"); if (!ctx) return;
    ...
    const PieLabels = { id:"pieLabels", afterDatasetDraw(chart){ const {ctx}=chart; const meta=chart.getDatasetMeta(0); const ds=chart.data.datasets[0].data; ctx.save(); meta.data.forEach((arc,i)=>{ const val=Number(ds[i]||0); if(!val) return; const pct = total ? (val/total)*100 : 0; if (pct<4) return; const label=chart.data.labels[i]; const angle=(arc.startAngle+arc.endAngle)/2; const r=arc.innerRadius+(arc.outerRadius-arc.innerRadius)*0.72; const x=arc.x+Math.cos(angle)*r; const y=arc.y+Math.sin(angle)*r; ctx.font="12px system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,'Helvetica Neue',Arial"; ctx.textAlign="center"; ctx.textBaseline="middle"; ctx.fillStyle="#111827"; const text= label && label.length>18 ? `${pct.toFixed(1)}%` : `${label}`; ctx.fillText(text,x,y); if (text!==`${pct.toFixed(1)}%`) { ctx.fillStyle="#6b7280"; ctx.fillText(`${pct.toFixed(1)}%`,x,y+12); } }); ctx.restore(); } };

    addChart(new Chart(ctx, { type:"pie", data:{ labels, datasets:[{ data, backgroundColor:bg }] }, options:{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{ position:"right", labels:{ usePointStyle:true, boxWidth:8 } }, tooltip:{ callbacks:{ label:(tt)=>{ const v=tt.parsed||0; const p = total? ((v/total)*100).toFixed(1) : "0.0"; return `${tt.label}: ${money(v)} (${p}%)`; } } } } }, plugins:[PieLabels] }));
  }

  function renderCategoryList(rows, total, monthsCount) {
    const box = document.getElementById("cat-list"); if (!box) return;
    const COLORS = ["#ef4444","#22c55e","#3b82f6","#0f766e","#f59e0b","#8b5cf6","#10b981","#f43f5e","#64748b","#e11d48"];
    box.innerHTML = rows.map((r,i)=> {
      ...
      return `<div class="cat-item">
        <div class="cat-left"><span class="cat-dot" style="background:${color}"></span>
          <div><div class="cat-name">${r.category}</div><div class="cat-meta">${pct.toFixed(1)}% do total de despesas</div></div>
        </div>
        <div class="cat-right"><div class="cat-amount">${money(r.total)}</div><div class="cat-avg">${money(avg)}/mês</div></div>
      </div>`;
    }).join("");
  }
